              ...
              {messages.map((msg, index) => {
                ...
                return (
                  <div key={index} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div
                      className={`max-w-[70%] px-4 py-2 rounded-2xl shadow-sm relative ${isMe ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-white rounded-bl-none'
                        }`}
                    >
                      {!isMe && (
                        <div className="text-[10px] text-gray-400 mb-1 opacity-75">{msg.sender?.username || msg.username || selectedUser?.username || 'User'}</div>
                      )}
